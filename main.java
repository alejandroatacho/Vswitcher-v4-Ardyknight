...
//Collaborated Methods Down Below:
    ...
    } else if (client.getBoostedSkillLevel(Skill.HITPOINTS) > minHp && ( v.getInventory().hasIn(foodId) || food_counter > 0)) {
       if (v.getLocalPlayer().hasAnimation(-1) || v.getLocalPlayer().hasAnimation(881) && client.getLocalPlayer().getGraphic() < 245) {
            if (client.getLocalPlayer().getInteracting() == null && v.getWalking().nearEntity(Entity.NPC, 3297, 10)) {
                v.invoke("Pickpocket","<col=ffff00>Knight of Ardougne<col=ff00> (level-46)",ardy_knight_id,11,0,0,false);
            }
        }
    }
